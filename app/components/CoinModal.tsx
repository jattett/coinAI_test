...
export default function CoinModal({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center" onClick={onClose}>
      <div
        className="bg-white p-8 rounded-2xl shadow-lg w-[800px] h-[600px] flex flex-col items-center"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-2xl font-bold mb-3 text-center">{coin.name} 가격 예측 (KRW)</h2>

        <div className="w-full h-[400px]">
          {predictedPrices.length === 0 ? (
            <p className="text-gray-500">⏳ AI 예측 중...</p>
          ) : (
            <Line
              data={{
                labels: [
